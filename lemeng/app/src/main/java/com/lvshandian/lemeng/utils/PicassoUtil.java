...
public class PicassoUtil {
     ...
     * 设置正在加载图片的默认图
     */
    private int load_image = R.mipmap.head_default;
    /**
     * 设置加载图片失败的默认图
     */
    private int load_error = R.mipmap.head_default;
     ...
     * @updateInfo (此处输入修改内容, 若无修改可不写.)
     */
    public void onLocadThumbnail(Context context, String path, int desWidth, int desHeight, ImageView imageView) {
        if (!TextUtils.isEmpty(path) && null != imageView) {
            Picasso.with(context).load(new File(path)).transform(new Thumformation(desWidth,desHeight)).config(Config.RGB_565).placeholder(load_image).error(load_error).into(imageView);
        }
    }
...
//                    .error(load_error).into(imageView);
            Picasso.with(context).load(path).transform(new RoundnessFormation()).placeholder(R.mipmap
                    .head_default).into(imageView);
        }
    }
    ...
    public class Thumformation implements Transformation {
        ...
        public Thumformation(int desWidth, int desHeight) {
        ...
        public Bitmap transform(Bitmap source) {
            // 缩略图的尺寸属性对象
            Bitmap result = Bitmap.createScaledBitmap(source, desWidth, desHeight, true);
            if (result != source) {
                source.recycle();
            }
            return result;
        ...
        public Bitmap transform(Bitmap source, int desWidth, int desHeight) {
            // 缩略图的尺寸属性对象
            Bitmap result = Bitmap.createScaledBitmap(source, desWidth, desHeight, true);
            if (result != source) {
                source.recycle();
