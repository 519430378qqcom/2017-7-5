package com.lvshandian.lemeng.third.wangyiyunxin.chatroom.viewholder;
...
//        LogUtil.e("ChatRoomViewHolderHelper接收到的数据：",message.getRemoteExtension().toString());
        if (message.getMsgType() != MsgTypeEnum.notification) {
            ...
            text.setTextColor(context.getResources().getColor(R.color.color_yellow_nick));
    ...
    private static void setNameIconView(ChatRoomMessage message, ImageView nameIconView) {
        ...
        MemberType type;
        if (ext.get(KEY) instanceof Integer)
            type = MemberType.typeOfValue((Integer) ext.get(KEY));
        else
            type = MemberType.typeOfValue(Integer.parseInt((String) ext.get(KEY)));
